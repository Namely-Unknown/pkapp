...
public class PlantDetailView {
	...
	private ArrayList<PlantTimeData> data;

	public void setDataList(List<OrderItem> itemList) {

		// Set up the array list and add five holders
		this.data = new ArrayList<PlantTimeData>();
		for (int i = 0; i < 5; i++) {
			this.data.add(new PlantTimeData());
		}
		...
		for (
			...
			PlantTimeData currentData = this.data.get(i);
			PlantTimeData lastData = this.data.get(i - 1);
	...
	public ArrayList<PlantTimeData> getData() {
		return data;
	}
	public void setData(ArrayList<PlantTimeData> data) {
		this.data = data;
	}
